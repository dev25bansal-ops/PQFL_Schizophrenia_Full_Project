...
ROOT = Path(r"F:\PQFL_Schizophrenia_Full_Project\pqfl_project\data")
OUT = ROOT / "data_inventory.txt"
L = []
def w(s=""): L.append(s)

w(f"DATA INVENTORY - {datetime.datetime.now()}")
w(f"Root: {ROOT}")
w()
w("=== TOP-LEVEL ENTRIES ===")
...
for ds in sorted(ROOT.iterdir()):
    if not ds.is_dir() or ds.name == "processed" or ds.name.startswith("."):
        continue
    w("=" * 80)
    w(f"DATASET: {ds.name}")
    w("=" * 80)
    ...
    w()
    w("--- Directory tree (depth 3, first 60 dirs) ---")
    seen = 0
    for d in sorted(ds.rglob("*")):
        ...
        seen += 1
    w()
    w("--- CSV/TSV/JSON/TXT files (first 15) ---")
    text_exts = {".csv", ".tsv", ".json", ".txt", ".md", ".yaml", ".yml", ".log", ".ini"}
    text_files = [f for f in files if f.suffix.lower() in text_exts][:15]
    for f in text_files:
        ...
        w()
    w("--- Sample sub-* folder ---")
    sub_dirs = list(ds.rglob("sub-*"))
    sub_dirs = [d for d in sub_dirs if d.is_dir()][:1]
    if sub_dirs:
        sd = sub_dirs[0]
        w(f"  {sd.relative_to(ROOT)}")
        for f in list(sd.rglob("*"))[:25]:
            if f.is_file():
                w(f"    ./{f.relative_to(sd).as_posix()}   {f.stat().st_size/1e6:.1f} MB")
    else:
...
if proc.exists():
    for f in sorted(proc.glob("*.npz")):
        w(f"  {f.name}   {f.stat().st_size/1e6:.1f} MB   {datetime.datetime.fromtimestamp(f.stat().st_mtime)}")
else:
